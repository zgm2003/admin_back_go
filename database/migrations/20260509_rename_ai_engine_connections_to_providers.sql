...
SET @drop_ai_agents_old_idx := (
  SELECT IF(COUNT(*) > 0, 'ALTER TABLE `ai_agents` DROP INDEX `idx_ai_agents_connection`', 'SELECT 1')
  FROM information_schema.statistics
  WHERE table_schema = @schema_name AND table_name = 'ai_agents' AND index_name = 'idx_ai_agents_connection'
);
PREPARE stmt FROM @drop_ai_agents_old_idx;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @rename_ai_agents_provider_id := (
  SELECT IF(
    EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema = @schema_name AND table_name = 'ai_agents' AND column_name = 'engine_connection_id')
    AND NOT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema = @schema_name AND table_name = 'ai_agents' AND column_name = 'provider_id'),
    'ALTER TABLE `ai_agents` CHANGE COLUMN `engine_connection_id` `provider_id` BIGINT UNSIGNED NOT NULL',
    'SELECT 1')
);
PREPARE stmt FROM @rename_ai_agents_provider_id;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
