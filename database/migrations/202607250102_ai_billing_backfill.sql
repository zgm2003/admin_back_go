...
UPDATE `ai_runs`
SET `request_fingerprint` = UNHEX(SHA2(CONCAT(
      'v1|user:', `user_id`, '|operation:', CASE WHEN `conversation_id` IS NULL THEN 'legacy_paid' ELSE 'chat' END,
      '|agent:', `agent_id`, '|model:', `model_id`, '|text:', `input_snapshot`, '|request:', `request_id`
    ), 256)),
...
SELECT IF(COUNT(*) = 0, 0, 1)
FROM (
  SELECT `id` FROM `ai_text_tasks` WHERE `request_id` IS NULL OR `request_fingerprint` IS NULL OR `run_id` IS NULL OR `run_id` = 0
  UNION ALL SELECT `id` FROM `ai_image_tasks` WHERE `request_id` IS NULL OR `request_fingerprint` IS NULL OR `run_id` IS NULL OR `run_id` = 0
  UNION ALL SELECT `id` FROM `ai_video_tasks` WHERE `request_id` IS NULL OR `request_fingerprint` IS NULL OR `run_id` IS NULL OR `run_id` = 0
