...
-- Contract DDL is destructive and MySQL implicitly commits each ALTER.  The
-- phase journal prevents a partial contract from being replayed blindly.
INSERT INTO `_ai_billing_contract_guard`
SELECT IF(COUNT(*) = 1, 0, 1)
FROM `ai_billing_migration_metadata`
WHERE `migration_key` = 'ai_billing_backfill_v1' AND `phase` = 'complete';

SET @ai_billing_contract_preexisting = (
  SELECT COUNT(*) FROM `ai_billing_migration_metadata`
  WHERE `migration_key` = 'ai_billing_contract_v1'
);
INSERT INTO `_ai_billing_contract_guard`
...
WHERE `migration_key` = 'ai_billing_contract_v1' AND `phase` = 'started';
