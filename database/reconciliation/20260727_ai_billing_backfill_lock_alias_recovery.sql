...
SET autocommit = 0;
LOCK TABLES `user_wallets` WRITE, `user_wallets` AS wallet WRITE, `wallet_transactions` WRITE;
...
FROM (
  ...
  WHERE task.`request_id` IS NULL OR task.`request_fingerprint` IS NULL OR task.`run_id` IS NULL OR task.`run_id` = 0
     OR task.`request_fingerprint` <> run_row.`request_fingerprint` OR task.`request_identity_status` <> 'legacy_non_replayable' OR BINARY task.`request_identity_marker` <> BINARY run_row.`request_identity_marker`
