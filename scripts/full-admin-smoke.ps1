...
function Get-ExpectedHttpFailureStatus([string]$Uri, [hashtable]$Headers, [string]$Label) {
  try {
    $response = Invoke-WebRequest -Uri $Uri -Method Get -Headers $Headers -UseBasicParsing -TimeoutSec 10
    return [int]$response.StatusCode
  } catch {
    if ($null -eq $_.Exception.Response) {
      throw "$Label did not return an HTTP response"
    }
...
try {
  ...
  $clientVersionUnauthenticatedStatus = Get-ExpectedHttpFailureStatus `
    "$baseURL/api/admin/v1/client-versions" `
    @{} `
    'retired client-version unauthenticated probe'
  if ($clientVersionUnauthenticatedStatus -ne 401) {
    throw "retired client-version unauthenticated probe returned $clientVersionUnauthenticatedStatus instead of 401"
  }
  $clientVersionAuthenticatedStatus = Get-ExpectedHttpFailureStatus `
    "$baseURL/api/admin/v1/client-versions" `
    $authHeaders `
    'retired client-version authenticated probe'
  if ($clientVersionAuthenticatedStatus -ne 404) {
    throw "retired client-version authenticated probe returned $clientVersionAuthenticatedStatus instead of route absence 404"
  }
  ...
  $summary = [ordered]@{
    ...
    client_version_unauthenticated_absence_status = $clientVersionUnauthenticatedStatus
    client_version_authenticated_absence_status = $clientVersionAuthenticatedStatus
