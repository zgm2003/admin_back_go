...
param(
  ...
  [Parameter(Mandatory = $true)]
  [ValidateSet('ledger', 'expand', 'backfill-core', 'backfill-ai', 'proven-indexes', 'all-nondestructive')]
  [string]$Stage,
...
$stageFiles = [ordered]@{
  ...
  'all-nondestructive' = @('001_ledger.sql', '010_expand_core.sql', '020_backfill_core.sql', '021_backfill_ai.sql', '041_apply_proven_indexes.sql')
}
