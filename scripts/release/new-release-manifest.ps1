...
$releaseDocument = [ordered]@{
  ...
  database = [ordered]@{
    atlas_version = [string]$proof.atlas_version
    target_fingerprint_sha256 = [string]$proof.target_fingerprint_sha256
    atlas_sum_sha256 = Get-FileSha256 -Path (Join-Path $script:BackendRoot 'database\migrations\atlas.sum')
  }
  evidence = [ordered]@{
