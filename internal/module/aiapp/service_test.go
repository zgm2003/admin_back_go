...
func (f *fakeAIAppRepository) ListActiveProviders(ctx context.Context) ([]Provider, error) {
	return f.connections, nil
}

func (f *fakeAIAppRepository) GetActiveProvider(ctx context.Context, id uint64) (*Provider, error) {
...
func TestCreateRejectsDuplicateCode(t *testing.T) {
	...
	_, appErr := service.Create(context.Background(), CreateInput{ProviderID: 1, Name: "客服助手", Code: "support_bot", AppType: "chat", DefaultResponseMode: "streaming", Status: enum.CommonYes})
...
func TestUpdateBlankEngineAppAPIKeyKeepsExistingCiphertext(t *testing.T) {
	...
	appErr := service.Update(context.Background(), 5, UpdateInput{ProviderID: 1, Name: "客服助手", Code: "support_bot", AppType: "chat", DefaultResponseMode: "streaming", Status: enum.CommonYes})
	if appErr != nil {
...
func TestTestDecryptsAppKeyAndUsesActiveProvider(t *testing.T) {
	...
	repo := &fakeAIAppRepository{
		rowByID: map[uint64]AppWithEngine{5: {
			App: App{ID: 5, ProviderID: 1, EngineAppAPIKeyEnc: cipher, Status: enum.CommonYes, IsDel: enum.CommonNo},
		}},
...
func TestTestReturnsUpstreamError(t *testing.T) {
	...
	repo := &fakeAIAppRepository{
		rowByID:         map[uint64]AppWithEngine{5: {App: App{ID: 5, ProviderID: 1, EngineAppAPIKeyEnc: cipher, Status: enum.CommonYes, IsDel: enum.CommonNo}}},
		activeProviders: map[uint64]Provider{1: {ID: 1, Name: "Dify", EngineType: "dify", BaseURL: "https://api.dify.test/v1", Status: enum.CommonYes, IsDel: enum.CommonNo}},
	}
