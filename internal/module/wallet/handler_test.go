...
func TestHandlerConsumeUsesCurrentIdentity(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeHTTPService{consumeResponse: &ConsumeResponse{Wallet: SummaryResponse{BalanceCents: 900}}}
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set(middleware.ContextAuthIdentity, &middleware.AuthIdentity{UserID: 7, SessionID: 1, Platform: "admin"})
	})
...
func (f *fakeHTTPService) Summary(ctx context.Context, userID int64) (*SummaryResponse, *apperror.Error) {
	f.summaryUserID = userID
	return &SummaryResponse{BalanceCents: 0, BalanceText: "0.00"}, nil
}
...
func (f *fakeHTTPService) Consume(ctx context.Context, input ConsumeInput) (*ConsumeResponse, *apperror.Error) {
...
func (f *fakeHTTPService) WalletUsersPageInit(ctx context.Context) (*WalletUsersPageInitResponse, *apperror.Error) {
	return &WalletUsersPageInitResponse{}, nil
}
func (f *fakeHTTPService) WalletUsers(ctx context.Context, query WalletUserListQuery) (*WalletUserListResponse, *apperror.Error) {
	return &WalletUserListResponse{}, nil
}
func (f *fakeHTTPService) LedgerPageInit(ctx context.Context) (*LedgerPageInitResponse, *apperror.Error) {
	return &LedgerPageInitResponse{}, nil
}
func (f *fakeHTTPService) Ledger(ctx context.Context, query TransactionListQuery) (*TransactionListResponse, *apperror.Error) {
	return &TransactionListResponse{}, nil
}
