...
type HTTPService interface {
	...
	Ledger(ctx context.Context, query walletmodule.TransactionListQuery) (*walletmodule.TransactionListResponse, *apperror.Error)
}
...
func (h *Handler) WalletUsers(c *gin.Context) {
	...
	result, appErr := h.requireService().WalletUsers(c.Request.Context(), walletmodule.WalletUserListQuery{CurrentPage: req.CurrentPage, PageSize: req.PageSize, Keyword: req.Keyword, UserID: req.UserID})
	writeResult(c, result, appErr)
}
...
func (h *Handler) Ledger(c *gin.Context) {
	...
	result, appErr := h.requireService().Ledger(c.Request.Context(), walletmodule.TransactionListQuery{CurrentPage: req.CurrentPage, PageSize: req.PageSize, UserID: req.UserID, Keyword: req.Keyword, Direction: req.Direction, SourceType: req.SourceType, DateStart: req.DateStart, DateEnd: req.DateEnd})
	writeResult(c, result, appErr)
}
...
func (failingService) WalletUsers(ctx context.Context, query walletmodule.WalletUserListQuery) (*walletmodule.WalletUserListResponse, *apperror.Error) {
	return nil, serviceNotConfigured()
}
func (failingService) LedgerPageInit(ctx context.Context) (*walletmodule.LedgerPageInitResponse, *apperror.Error) {
	return nil, serviceNotConfigured()
}
func (failingService) Ledger(ctx context.Context, query walletmodule.TransactionListQuery) (*walletmodule.TransactionListResponse, *apperror.Error) {
	return nil, serviceNotConfigured()
}
