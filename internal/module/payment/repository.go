...
func (r *GormRepository) ListConfigs(ctx context.Context, query ConfigListQuery) ([]AlipayConfig, int64, error) {
	if r == nil || r.db == nil {
		return nil, 0, ErrRepositoryNotConfigured
	}
	_, limit, offset := normalizePage(query.CurrentPage, query.PageSize)
	db := r.db.WithContext(ctx).Model(&AlipayConfig{}).Where("is_del = ?", enum.CommonNo)
	...
	var rows []AlipayConfig
	err := db.Order("id desc").Limit(limit).Offset(offset).Find(&rows).Error
	return rows, total, err
}

func (r *GormRepository) GetConfig(ctx context.Context, id int64) (*AlipayConfig, error) {
	if r == nil || r.db == nil {
		return nil, ErrRepositoryNotConfigured
	}
	var row AlipayConfig
...
func (r *GormRepository) GetConfigByCode(ctx context.Context, code string) (*AlipayConfig, error) {
	if r == nil || r.db == nil {
		return nil, ErrRepositoryNotConfigured
	}
	var row AlipayConfig
...
func (r *GormRepository) CreateConfig(ctx context.Context, cfg AlipayConfig) (int64, error) {
	if r == nil || r.db == nil {
...
func (r *GormRepository) UpdateConfig(ctx context.Context, cfg AlipayConfig, keepPrivateKey bool) error {
	if r == nil || r.db == nil {
	...
	fields := map[string]any{
		...
		"alipay_cert_path":      cfg.AlipayCertPath,
		"alipay_root_cert_path": cfg.AlipayRootCertPath,
	...
	if !keepPrivateKey {
		fields["app_private_key_enc"] = cfg.AppPrivateKeyEnc
		fields["app_private_key_hint"] = cfg.AppPrivateKeyHint
	}
	return r.db.WithContext(ctx).Model(&AlipayConfig{}).Where("id = ? AND is_del = ?", cfg.ID, enum.CommonNo).Updates(fields).Error
}
...
func (r *GormRepository) ChangeConfigStatus(ctx context.Context, id int64, status int) error {
	...
	return r.db.WithContext(ctx).Model(&AlipayConfig{}).Where("id = ? AND is_del = ?", id, enum.CommonNo).Update("status", status).Error
}
...
func (r *GormRepository) DeleteConfig(ctx context.Context, id int64) error {
	...
	return r.db.WithContext(ctx).Model(&AlipayConfig{}).Where("id = ? AND is_del = ?", id, enum.CommonNo).Updates(map[string]any{
