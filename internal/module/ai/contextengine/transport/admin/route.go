...
func RegisterRoutes(router gin.IRoutes, trustedPlatform string, service HTTPService, registries ...*adminroute.Registry) {
	...
	read := func(method, path, operation, permission, responseSchema string, contract any, handlerFunc gin.HandlerFunc) {
		register(adminroute.Definition{Method: method, Path: path, OperationID: operation, Access: adminroute.Permission(permission), Audit: adminroute.NoAudit("read-only"), ResponseSchema: responseSchema, Contract: &adminroute.HTTPContract{Query: contract, Response: responseSchema}}, handlerFunc)
	}
	_ = read
	...
	register(adminroute.Definition{Method: http.MethodPost, Path: "/api/admin/v1/ai/context-documents/:id/reindex", OperationID: "ai_context_document_reindex", Access: adminroute.Permission("ai_context_document_manage"), Audit: adminroute.Audit("ai_context_document", "reindex", "重建上下文文档索引"), Contract: &adminroute.HTTPContract{Response: contextengine.DocumentAdminDTO{}}}, handler.ReindexDocument)

	register(adminroute.Definition{Method: http.MethodPost, Path: "/api/admin/v1/ai/context-evaluations", OperationID: "ai_context_evaluate", Access: adminroute.Permission("ai_context_evaluate"), Audit: adminroute.Audit("ai_context", "evaluate", "执行上下文评测"), Contract: &adminroute.HTTPContract{Request: evaluationRequest{}, Response: contextengine.ContextEvaluationResponse{}}}, handler.Evaluate)
