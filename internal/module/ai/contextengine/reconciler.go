...
func NewDocumentIndexReconciler(repository IngestionRepository, enqueuer *QueueDocumentVersionEnqueuer, batchSize int, maxRetry uint32) *DocumentIndexReconciler {
	return &DocumentIndexReconciler{repository: repository, enqueuer: enqueuer, batchSize: batchSize, maxRetry: maxRetry}
}
...
func (reconciler *DocumentIndexReconciler) reconcile(ctx context.Context, now time.Time) (bool, error) {
	if reconciler == nil || reconciler.repository == nil || reconciler.enqueuer == nil || reconciler.enqueuer.queue == nil || reconciler.batchSize <= 0 || reconciler.maxRetry == 0 {
	...
	for _, candidate := range candidates {
		if candidate.State == DocumentVersionProcessing && candidate.AttemptCount >= reconciler.maxRetry {
			if err := reconciler.repository.FinalizeVersion(ctx, candidate.VersionID, "ai.context.index_retry_exhausted", reconciler.maxRetry, now); err != nil {
				return false, err
			}
			continue
		}
