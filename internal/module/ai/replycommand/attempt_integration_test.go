...
func TestProviderAttemptPersistsBeforeDispatchAndRequiresCurrentFence(t *testing.T) {
	...
	if attempt, ok, err := repository.PrepareAttempt(ctx, PrepareAttemptInput{CommandID: created.CommandID, Owner: "stale-worker", Token: claim.FencingToken, Now: now}); err != nil || ok || attempt != nil {
		t.Fatalf("stale attempt=%+v ok=%v err=%v", attempt, ok, err)
	}
	first, ok, err := repository.PrepareAttempt(ctx, PrepareAttemptInput{CommandID: created.CommandID, Owner: claim.Owner, Token: claim.FencingToken, Now: now})
	...
	second, ok, err := repository.PrepareAttempt(ctx, PrepareAttemptInput{CommandID: created.CommandID, Owner: claim.Owner, Token: claim.FencingToken, Now: now.Add(time.Second)})
...
func TestExpiredDispatchedAttemptBecomesOutcomeUnknownInsteadOfBlindRetry(t *testing.T) {
	...
	attempt, ok, err := repository.PrepareAttempt(ctx, PrepareAttemptInput{CommandID: created.CommandID, Owner: claim.Owner, Token: claim.FencingToken, Now: now})
	if err != nil || !ok {
