	...
	err := repository.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&PriceOverride{}).
			...
			Updates(map[string]any{
				"version": gorm.Expr("version + 1"), "source_url": command.SourceURL,
				"verified_at": command.VerifiedAt, "updated_by": command.UpdatedBy,
			})
		if result.Error != nil {
			return result.Error
		}
...
func findOverride(db *gorm.DB, catalogVendor string, modelID string, locked bool) (*PriceOverride, error) {
	...
	row := rows[0]
	if err := db.Where("override_id = ?", row.ID).Order("category ASC, unit ASC, tier_key ASC, id ASC").Find(&row.Rates).Error; err != nil {
		return nil, err
	}
	return &row, nil
}
