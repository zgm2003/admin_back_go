...
type HTTPService interface {
	PageInit(ctx context.Context) (*assetmodule.PageInitResponse, *apperror.Error)
	List(ctx context.Context, query assetmodule.ListQuery) (*assetmodule.ListResponse, *apperror.Error)
	Detail(ctx context.Context, id int64) (*assetmodule.Item, *apperror.Error)
...
func (h *Handler) List(c *gin.Context) {
	...
	result, appErr := h.requireService().List(c.Request.Context(), assetmodule.ListQuery{CurrentPage: req.CurrentPage, PageSize: req.PageSize, Keyword: req.Keyword, Type: req.Type, Status: req.Status})
	writeResult(c, result, appErr)
}
...
func (h *Handler) DeleteBatch(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, apperror.BadRequestKey("ai.asset.ids.invalid", nil, "素材ID列表无效"))
		return
	}
	...
	response.OKWithMessageKey(c, gin.H{}, "ai.asset.delete_batch_success", nil, "批量删除AI素材成功")
}
...
func (failingService) PageInit(ctx context.Context) (*assetmodule.PageInitResponse, *apperror.Error) {
	return nil, apperror.InternalKey("ai.asset.service_missing", nil, "AI素材服务未配置")
}
func (failingService) List(ctx context.Context, query assetmodule.ListQuery) (*assetmodule.ListResponse, *apperror.Error) {
	return nil, apperror.InternalKey("ai.asset.service_missing", nil, "AI素材服务未配置")
}
func (failingService) Detail(ctx context.Context, id int64) (*assetmodule.Item, *apperror.Error) {
	return nil, apperror.InternalKey("ai.asset.service_missing", nil, "AI素材服务未配置")
}
...
func input(req assetRequest) assetmodule.Input {
	return assetmodule.Input{Slug: req.Slug, Type: req.Type, Category: req.Category, Title: req.Title, CoverURL: req.CoverURL, Description: req.Description, Content: req.Content, URL: req.URL, TagsJSON: req.TagsJSON, Status: req.Status}
}
