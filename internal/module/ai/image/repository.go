...
func (r *GormRepository) ListImageAgents(ctx context.Context, scene string) ([]AgentOption, error) {
	...
	err := r.db.WithContext(ctx).Table("ai_agents AS a").
		...
		Joins("JOIN ai_provider_models AS m ON m.id = a.provider_model_id AND m.provider_id = a.provider_id AND m.model_id = a.model_id AND m.model_kind = ? AND m.status = ? AND m.mapping_status = ?", aiprovider.ModelKindImage, enum.CommonYes, officialmodel.MappingStatusMapped).
