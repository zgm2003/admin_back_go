...
func (f *fakeRepository) UpdateTask(ctx context.Context, platform string, userID int64, id int64, fields map[string]any) error {
	f.updates = append(f.updates, updateCall{platform: platform, userID: userID, id: id, fields: fields})
	return f.updateErr
}

func (f *fakeRepository) GetTask(ctx context.Context, platform string, userID int64, id int64) (*VideoTask, error) {
	f.getPlatform = platform
	f.getUserID = userID
	f.getID = id
	return f.task, nil
}
...
func validGenerationVideoAgent(t *testing.T, box secretbox.Box) *AgentRuntime {
	t.Helper()
	cipher, err := box.Encrypt("provider-key")
	if err != nil {
		t.Fatalf("encrypt fixture: %v", err)
	}
	return &AgentRuntime{AgentID: 8, ProviderID: 9, ModelID: "grok-imagine-video", ScenesJSON: `["video_generate"]`, EngineType: string(infraai.EngineTypeOpenAI), EngineBaseURL: "https://api.openai.test/v1", EngineAPIKeyEnc: cipher, AgentStatus: enum.CommonYes, EngineStatus: enum.CommonYes}
}
...
func TestVideoTaskUsesCanonicalTable(t *testing.T) {
	if got := (VideoTask{}).TableName(); got != "ai_video_tasks" {
		t.Fatalf("expected ai_video_tasks table, got %q", got)
	}
}
...
func TestUploadReferenceMediaStoresCOSObjectAndReturnsProviderURL(t *testing.T) {
	...
	result, appErr := service.UploadReferenceMedia(context.Background(), ReferenceMediaUploadInput{Platform: enum.PlatformAdmin,
	...
	if !strings.HasPrefix(result.StorageKey, "ai-video-references/admin/video/2026/06/09/7-") || !strings.HasSuffix(result.StorageKey, ".mp4") {
...
func TestUploadReferenceMediaRejectsInvalidInput(t *testing.T) {
	...
	cases := []ReferenceMediaUploadInput{
		{Platform: enum.PlatformAdmin, UserID: 0, MediaKind: "video", FileName: "reference.mp4", Body: []byte("video")},
		{Platform: enum.PlatformAdmin, UserID: 7, MediaKind: "text", FileName: "reference.txt", Body: []byte("text")},
		{Platform: enum.PlatformAdmin, UserID: 7, MediaKind: "video", FileName: "empty.mp4", Body: nil},
		{Platform: enum.PlatformAdmin, UserID: 7, MediaKind: "audio", FileName: "reference.txt", MimeType: "text/plain", Body: []byte("audio")},
	}
...
func TestUploadReferenceMediaRejectsMissingOrUnregisteredPlatform(t *testing.T) {
	service := NewService(Dependencies{})
	for _, platform := range []string{"", "partner_portal"} {
		_, appErr := service.UploadReferenceMedia(context.Background(), ReferenceMediaUploadInput{Platform: platform, UserID: 7, MediaKind: "video", FileName: "reference.mp4", Body: []byte("video")})
		if appErr == nil || appErr.MessageID != "aivideo.platform.invalid" {
...
func TestCreateUsesAgentModelCreatesLocalTaskAndStoresProviderTask(t *testing.T) {
	...
	repo := &fakeRepository{agent: validGenerationVideoAgent(t, box)}
	recorder := &fakeRunRecorder{nextID: 99}

	generateAudio := false
	watermark := true
	result, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: factory, RunRecorder: recorder}).Create(context.Background(), CreateInput{Platform: enum.PlatformAdmin, UserID: 7, AgentID: 8, ModelID: "client-model", Prompt: " clip ", DurationSeconds: 4, Size: "1280x720", ResolutionName: "720p", GenerateAudio: &generateAudio, Watermark: &watermark})
	...
	if repo.createdTask.Platform != enum.PlatformAdmin || repo.createdTask.UserID != 7 || repo.createdTask.AgentID != 8 || repo.createdTask.ModelID != "grok-imagine-video" || repo.createdTask.Prompt != "clip" || repo.createdTask.Status != StatusPending || repo.createdTask.IsDel != IsDelActive {
	...
	if len(repo.updates) != 2 || repo.updates[0].platform != enum.PlatformAdmin || repo.updates[0].userID != 7 || repo.updates[0].id != 77 || repo.updates[0].fields["run_id"] != int64(99) || repo.updates[1].platform != enum.PlatformAdmin || repo.updates[1].userID != 7 || repo.updates[1].id != 77 || repo.updates[1].fields["provider_task_id"] != "provider-task-1" || repo.updates[1].fields["status"] != StatusRunning {
		t.Fatalf("provider task update mismatch: %#v", repo.updates)
	}
	if recorder.started.Platform != enum.PlatformAdmin || recorder.started.RequestID != "ai_video_task_77" || recorder.started.InputSnapshot != "clip" {
...
func TestCreateRejectsMissingOrUnregisteredPlatformBeforeRepository(t *testing.T) {
	for _, platform := range []string{"", "partner_portal"} {
		repo := &fakeRepository{}
		_, appErr := NewService(Dependencies{Repository: repo}).Create(context.Background(), CreateInput{Platform: platform, UserID: 7, AgentID: 8, Prompt: "clip"})
		if appErr == nil || appErr.MessageID != "aivideo.platform.invalid" {
			t.Fatalf("expected platform %q to be rejected, got %#v", platform, appErr)
		}
		if repo.agentID != 0 || repo.createdTask.ID != 0 {
			t.Fatalf("invalid platform %q reached repository: %#v", platform, repo)
		}
	}
}
...
func TestCreateProviderFailureMarksLocalTaskFailed(t *testing.T) {
	...
	_, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: engine}, RunRecorder: recorder}).Create(context.Background(), CreateInput{Platform: enum.PlatformAdmin, UserID: 7, AgentID: 8, Prompt: "clip"})

	if appErr == nil || appErr.MessageID != "aivideo.provider_failed" {
	...
	if recorder.failed.RunID != 99 || recorder.failed.Message != "AI视频生成失败" {
...
func TestCreateReturnsTaskUpdateFailedWhenProviderFailureCannotMarkFailed(t *testing.T) {
	box := secretbox.New([]byte("12345678901234567890123456789012"))
	repo := &fakeRepository{agent: validGenerationVideoAgent(t, box), updateErr: errors.New("update failed")}
	engine := &fakeVideoEngine{createErr: errors.New("provider down")}

	_, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: engine}, RunRecorder: &fakeRunRecorder{nextID: 99}}).Create(context.Background(), CreateInput{Platform: enum.PlatformAdmin, UserID: 7, AgentID: 8, Prompt: "clip"})

	if appErr == nil || appErr.MessageID != "aivideo.task_update_failed" {
...
func TestCreateRejectsEmptyProviderTaskID(t *testing.T) {
	...
	if appErr == nil || appErr.MessageID != "aivideo.provider_task_invalid" {
...
func TestCreateRejectsUnknownProviderStatus(t *testing.T) {
	...
	for _, providerStatus := range []string{"paused", " "} {
		t.Run(providerStatus, func(t *testing.T) {
			...
			if appErr == nil || appErr.MessageID != "aivideo.provider_status_invalid" {
...
func TestStatusAndContentUseOwnedActiveTaskProviderTaskID(t *testing.T) {
	...
	repo := &fakeRepository{agent: validGenerationVideoAgent(t, box), task: &VideoTask{Platform: enum.PlatformAdmin, ID: 77, UserID: 7, AgentID: 8, ProviderTaskID: "provider-task-1", RunID: 99, Status: StatusRunning, IsDel: IsDelActive}}
	...
	status, appErr := svc.Status(context.Background(), enum.PlatformAdmin, 7, 77)
	if appErr != nil || status == nil || status.ID != 77 || status.Status != StatusCompleted || engine.statusID != "provider-task-1" || repo.getPlatform != enum.PlatformAdmin || repo.getUserID != 7 || repo.getID != 77 {
	...
	body, contentType, appErr := svc.Content(context.Background(), enum.PlatformAdmin, 7, 77)
	if appErr != nil || string(body) != "video" || contentType != "video/mp4" || engine.contentID != "provider-task-1" || repo.getPlatform != enum.PlatformAdmin || repo.getUserID != 7 || repo.getID != 77 {
...
func TestStatusReturnsTaskUpdateFailedWhenPersistingProviderStatusFails(t *testing.T) {
	...
	repo := &fakeRepository{agent: validGenerationVideoAgent(t, box), task: &VideoTask{Platform: enum.PlatformAdmin, ID: 77, UserID: 7, AgentID: 8, ProviderTaskID: "provider-task-1", RunID: 99, Status: StatusRunning, IsDel: IsDelActive}, updateErr: errors.New("update failed")}

	_, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: engine}}).Status(context.Background(), enum.PlatformAdmin, 7, 77)
...
func TestStatusCancelsVideoRunWhenProviderCancelled(t *testing.T) {
	...
	status, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: engine}, RunRecorder: recorder}).Status(context.Background(), enum.PlatformAdmin, 7, 77)
...
func TestStatusRejectsUnknownProviderStatus(t *testing.T) {
	...
	for _, providerStatus := range []string{"paused", " "} {
		t.Run(providerStatus, func(t *testing.T) {
			...
			repo := &fakeRepository{agent: validGenerationVideoAgent(t, box), task: &VideoTask{Platform: enum.PlatformAdmin, ID: 77, UserID: 7, AgentID: 8, ProviderTaskID: "provider-task-1", Status: StatusRunning, IsDel: IsDelActive}}
...
func TestStatusRejectsTaskFromDifferentUser(t *testing.T) {
	box := secretbox.New([]byte("12345678901234567890123456789012"))
	repo := &fakeRepository{agent: validGenerationVideoAgent(t, box), task: &VideoTask{Platform: enum.PlatformAdmin, ID: 77, UserID: 99, AgentID: 8, ProviderTaskID: "provider-task-1", IsDel: IsDelActive}}

	_, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: &fakeVideoEngine{}}}).Status(context.Background(), enum.PlatformAdmin, 7, 77)

	if appErr == nil || appErr.MessageID != "aivideo.not_found" {
...
func TestContentRejectsMissingProviderTaskID(t *testing.T) {
	box := secretbox.New([]byte("12345678901234567890123456789012"))
	repo := &fakeRepository{agent: validGenerationVideoAgent(t, box), task: &VideoTask{Platform: enum.PlatformAdmin, ID: 77, UserID: 7, AgentID: 8, ProviderTaskID: " ", IsDel: IsDelActive}}

	_, _, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: &fakeVideoEngine{}}}).Content(context.Background(), enum.PlatformAdmin, 7, 77)

	if appErr == nil || appErr.MessageID != "aivideo.provider_task_missing" {
...
func TestContentRejectsEmptyProviderBody(t *testing.T) {
	box := secretbox.New([]byte("12345678901234567890123456789012"))
	repo := &fakeRepository{agent: validGenerationVideoAgent(t, box), task: &VideoTask{Platform: enum.PlatformAdmin, ID: 77, UserID: 7, AgentID: 8, ProviderTaskID: "provider-task-1", IsDel: IsDelActive}}

	_, _, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: &fakeVideoEngine{body: []byte{}, contentType: "video/mp4"}}}).Content(context.Background(), enum.PlatformAdmin, 7, 77)

	if appErr == nil || appErr.MessageID != "aivideo.content_empty" {
		t.Fatalf("expected empty content error, got %#v", appErr)
	}
}

func TestCreateRejectsNonGenerationVideoScene(t *testing.T) {
	box := secretbox.New([]byte("12345678901234567890123456789012"))
	agent := validGenerationVideoAgent(t, box)
	agent.ScenesJSON = `["chat"]`

	_, appErr := NewService(Dependencies{Repository: &fakeRepository{agent: agent}, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: &fakeVideoEngine{}}}).Create(context.Background(), CreateInput{Platform: enum.PlatformAdmin, UserID: 7, AgentID: 8, Prompt: "clip"})

	if appErr == nil || appErr.LegacyCode != apperror.CodeBadRequest || appErr.MessageID != "aivideo.agent_unavailable" {
		t.Fatalf("expected generation video scene rejection, got %#v", appErr)
	}
}
...
func TestCreateRejectsDisabledAgentAsUnavailable(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT a.id AS agent_id,\n\t\t\ta.provider_id AS provider_id,\n\t\t\ta.model_id AS model_id,\n\t\t\ta.model_display_name AS model_display_name,\n\t\t\ta.system_prompt AS system_prompt,\n\t\t\ta.scenes_json AS scenes_json,\n\t\t\ta.status AS agent_status,\n\t\t\te.engine_type AS engine_type,\n\t\t\te.base_url AS engine_base_url,\n\t\t\te.api_key_enc AS engine_api_key_enc,\n\t\t\te.status AS engine_status FROM ai_agents AS a JOIN ai_providers e ON e.id = a.provider_id AND e.is_del = ? WHERE a.id = ? AND a.is_del = ? LIMIT ?")).
		...
		WillReturnRows(sqlmock.NewRows([]string{"agent_id", "provider_id", "model_id", "model_display_name", "system_prompt", "scenes_json", "agent_status", "engine_type", "engine_base_url", "engine_api_key_enc", "engine_status"}).
			AddRow(int64(8), int64(9), "grok-imagine-video", "Video", "", `["video_generate"]`, enum.CommonNo, string(infraai.EngineTypeOpenAI), "https://api.openai.test/v1", cipher, enum.CommonYes))

	_, appErr := NewService(Dependencies{Repository: repo, Secretbox: box, EngineFactory: &fakeEngineFactory{engine: &fakeVideoEngine{}}}).Create(context.Background(), CreateInput{Platform: enum.PlatformAdmin, UserID: 7, AgentID: 8, Prompt: "clip"})

	if appErr == nil || appErr.MessageID != "aivideo.agent_unavailable" {
...
func TestCreateRejectsInvalidRequest(t *testing.T) {
	box := secretbox.New([]byte("12345678901234567890123456789012"))
	cases := []CreateInput{
		{Platform: enum.PlatformAdmin, UserID: 0, AgentID: 8, Prompt: "clip"},
		{Platform: enum.PlatformAdmin, UserID: 7, AgentID: 0, Prompt: "clip"},
		{Platform: enum.PlatformAdmin, UserID: 7, AgentID: 8, Prompt: "   "},
	}
	for _, input := range cases {
		...
		if appErr == nil || appErr.MessageID != "aivideo.request.invalid" {
