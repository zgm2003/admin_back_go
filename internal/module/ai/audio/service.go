...
func (s *Service) Generate(ctx context.Context, input GenerateInput) (*GenerateResponse, *apperror.Error) {
	...
	if s.runRecorder == nil {
		return nil, apperror.InternalKey("canvas.ai.audio.run_recorder_missing", nil, "Canvas音频运行记录服务未配置")
	}
	startedAt := s.now()
	...
	if err != nil {
		return nil, apperror.WrapKey(apperror.CodeInternal, 500, "canvas.ai.audio.run_start_failed", nil, "创建Canvas音频运行记录失败", err)
	}
	...
	if err != nil {
		finishedAt := s.now()
		_ = s.runRecorder.Fail(context.Background(), airun.FailInput{RunID: runID, Message: "Canvas音频生成失败", FinishedAt: finishedAt, DurationMS: durationMS(startedAt, finishedAt)})
		return nil, apperror.WrapKey(apperror.CodeInternal, 500, "canvas.ai.audio.provider_failed", nil, "Canvas音频生成失败", err)
	}
	if result == nil || len(result.Body) == 0 {
		finishedAt := s.now()
		_ = s.runRecorder.Fail(context.Background(), airun.FailInput{RunID: runID, Message: "Canvas音频内容为空", FinishedAt: finishedAt, DurationMS: durationMS(startedAt, finishedAt)})
		return nil, apperror.BadRequestKey("canvas.ai.audio.content_empty", nil, "Canvas音频内容为空")
	}
	...
	if err := s.runRecorder.Complete(context.Background(), airun.CompleteInput{RunID: runID, FinishedAt: finishedAt, DurationMS: durationMS(startedAt, finishedAt)}); err != nil {
		return nil, apperror.WrapKey(apperror.CodeInternal, 500, "canvas.ai.audio.run_complete_failed", nil, "更新Canvas音频运行记录失败", err)
	}
...
func normalizeInput(input GenerateInput) (GenerateInput, *apperror.Error) {
	...
	if input.AgentID <= 0 || input.Prompt == "" {
		return input, apperror.BadRequestKey("canvas.ai.audio.request.invalid", nil, "音频生成参数错误")
	}
	if input.Voice == "" {
...
func (s *Service) validAudioAgent(ctx context.Context, repo Repository, agentID int64) (*AgentRuntime, *apperror.Error) {
	...
	if err != nil {
		return nil, apperror.WrapKey(apperror.CodeInternal, 500, "canvas.ai.audio.agent_query_failed", nil, "查询音频智能体失败", err)
	}
	if agent == nil || agent.AgentID <= 0 {
		return nil, apperror.NotFoundKey("canvas.ai.audio.agent_not_found", nil, "音频智能体不存在")
	}
	if agent.AgentStatus != enum.CommonYes || agent.EngineStatus != enum.CommonYes || !supportsScene(agent.ScenesJSON, SceneCanvasAudioGenerate) {
		return nil, apperror.BadRequestKey("canvas.ai.audio.agent_unavailable", nil, "该智能体不支持音频生成")
	}
	if strings.TrimSpace(agent.EngineAPIKeyEnc) == "" {
		return nil, apperror.BadRequestKey("canvas.ai.audio.provider_key_missing", nil, "AI供应商API Key未配置")
	}
	if strings.TrimSpace(agent.ModelID) == "" {
		return nil, apperror.BadRequestKey("canvas.ai.audio.model_missing", nil, "音频智能体模型未配置")
	}
	return agent, nil
}
...
func (s *Service) engine(ctx context.Context, agent AgentRuntime) (infraai.AudioEngine, *apperror.Error) {
	if s == nil || s.secretbox == nil {
		return nil, apperror.InternalKey("canvas.ai.audio.secretbox_missing", nil, "AI密钥服务未配置")
	}
	apiKey, err := s.secretbox.Decrypt(agent.EngineAPIKeyEnc)
	if err != nil {
		return nil, apperror.WrapKey(apperror.CodeInternal, 500, "canvas.ai.audio.provider_key_decrypt_failed", nil, "解密AI供应商API Key失败", err)
	}
	if strings.TrimSpace(apiKey) == "" {
		return nil, apperror.BadRequestKey("canvas.ai.audio.provider_key_missing", nil, "AI供应商API Key未配置")
	}
	if s.engineFactory == nil {
		return nil, apperror.InternalKey("canvas.ai.audio.engine_missing", nil, "AI音频引擎工厂未配置")
	}
	...
	if err != nil {
		return nil, apperror.WrapKey(apperror.CodeInternal, 500, "canvas.ai.audio.engine_create_failed", nil, "创建AI音频引擎失败", err)
	}
	if engine == nil {
		return nil, apperror.InternalKey("canvas.ai.audio.engine_missing", nil, "AI音频引擎未配置")
	}
	return engine, nil
}
...
func (s *Service) requireRepository() (Repository, *apperror.Error) {
	if s == nil || s.repository == nil {
		return nil, apperror.InternalKey("canvas.ai.audio.repository_missing", nil, "Canvas音频生成仓储未配置")
	}
...
func normalizeContentType(contentType string, format string) (string, *apperror.Error) {
	...
	return "", apperror.BadRequestKey("canvas.ai.audio.content_type_invalid", nil, "Canvas音频内容类型无效")
}
...
func audioRunRequestID(now time.Time) string {
	return "canvas_audio_" + strconv.FormatInt(now.UnixNano(), 10)
}
