...
type fakeManagementService struct {
	initResult  *permissionmodule.InitResponse
	listResult  []permissionmodule.PermissionListItem
	createdID   int64
	err         *apperror.Error
	listQuery   permissionmodule.PermissionListQuery
	createInput permissionmodule.PermissionMutationInput
	updateID    int64
	updateInput permissionmodule.PermissionMutationInput
	deleteIDs   []int64
	statusID    int64
	statusValue int
}
...
func (f *fakeManagementService) List(ctx context.Context, query permissionmodule.PermissionListQuery) ([]permissionmodule.PermissionListItem, *apperror.Error) {
	f.listQuery = query
	return f.listResult, f.err
}

func (f *fakeManagementService) Create(ctx context.Context, input permissionmodule.PermissionMutationInput) (int64, *apperror.Error) {
	f.createInput = input
	return f.createdID, f.err
}

func (f *fakeManagementService) Update(ctx context.Context, id int64, input permissionmodule.PermissionMutationInput) *apperror.Error {
	f.updateID = id
	f.updateInput = input
	return f.err
}
...
func TestPermissionRoutesUseRESTfulMethods(t *testing.T) {
	service := &fakeManagementService{
		initResult: &permissionmodule.InitResponse{Dict: permissionmodule.PermissionDict{}},
		listResult: []permissionmodule.PermissionListItem{{ID: 1, Name: "系统"}},
		createdID:  9,
	}
	...
	assertPermissionStatus(t, router, http.MethodPatch, "/api/admin/v1/permissions/9/status", map[string]int{"status": permissionmodule.CommonNo}, http.StatusOK)
	...
	if service.createInput.Type != permissionmodule.TypeDir || service.createInput.Name != "目录" {
		t.Fatalf("create input mismatch: %#v", service.createInput)
	}
	if service.updateID != 9 || service.updateInput.Type != permissionmodule.TypePage {
		t.Fatalf("update input mismatch: id=%d input=%#v", service.updateID, service.updateInput)
	}
	if service.statusID != 9 || service.statusValue != permissionmodule.CommonNo {
...
func TestPermissionHandlerRejectsInvalidRouteID(t *testing.T) {
	...
	assertPermissionStatus(t, router, http.MethodPut, "/api/admin/v1/permissions/bad", permissionRequestBody(permissionmodule.TypeDir), http.StatusBadRequest)
}
...
func permissionRequestBody(permissionType int) map[string]any {
	...
	case permissionmodule.TypeDir:
		body["i18n_key"] = "menu.system"
		body["show_menu"] = permissionmodule.CommonYes
	case permissionmodule.TypePage:
		body["name"] = "页面"
		body["path"] = "/system/user"
		body["component"] = "system/user/index"
		body["i18n_key"] = "menu.system_user"
		body["show_menu"] = permissionmodule.CommonYes
	case permissionmodule.TypeButton:
		body["name"] = "新增"
		body["parent_id"] = 2
