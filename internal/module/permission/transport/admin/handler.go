...
func (h *ManagementHandler) List(c *gin.Context) {
	...
	result, appErr := h.service.List(c.Request.Context(), permissionmodule.PermissionListQuery{
...
func bindPermissionMutation(c *gin.Context) (permissionmodule.PermissionMutationInput, bool) {
	var req permissionMutationRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, apperror.BadRequest("参数错误"))
		return permissionmodule.PermissionMutationInput{}, false
	}
	return permissionmodule.PermissionMutationInput{
		Platform:  req.Platform,
		Type:      req.Type,
		Name:      req.Name,
