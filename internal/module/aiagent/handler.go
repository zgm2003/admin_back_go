...
func (h *Handler) List(c *gin.Context) {
	...
	result, appErr := h.requireService().List(c.Request.Context(), ListQuery{CurrentPage: req.CurrentPage, PageSize: req.PageSize, Name: req.Name, ModelID: req.ModelID, Mode: req.Mode, Status: req.Status})
	writeResult(c, result, appErr)
}
...
func routeID(c *gin.Context) (int64, bool) {
	...
	if err != nil || id <= 0 {
		response.Error(c, apperror.BadRequest("无效的AI智能体ID"))
		return 0, false
	}
	return id, true
}
...
func mutationInput(req mutationRequest) MutationInput {
	return MutationInput{Name: req.Name, ModelID: req.ModelID, Avatar: req.Avatar, SystemPrompt: req.SystemPrompt, Mode: req.Mode, Scene: req.Scene, Capabilities: req.Capabilities, SceneCodes: req.SceneCodes, RuntimeConfig: req.RuntimeConfig, Policy: req.Policy, Status: req.Status, ToolIDs: req.ToolIDs, KnowledgeBaseIDs: req.KnowledgeBaseIDs}
}
...
func (nilHTTPService) Create(ctx context.Context, input MutationInput) (int64, *apperror.Error) {
	return 0, apperror.Internal("AI智能体服务未配置")
}
func (nilHTTPService) Update(ctx context.Context, id int64, input MutationInput) *apperror.Error {
