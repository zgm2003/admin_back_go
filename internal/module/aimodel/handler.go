...
func (h *Handler) List(c *gin.Context) {
	...
	if err := c.ShouldBindQuery(&req); err != nil {
		response.Error(c, apperror.BadRequest("AI模型列表参数错误"))
		return
	}
...
func (h *Handler) Create(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, apperror.BadRequest("AI模型参数错误"))
		return
	}
...
func (h *Handler) ChangeStatus(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, apperror.BadRequest("AI模型状态参数错误"))
		return
	}
...
func routeID(c *gin.Context) (int64, bool) {
	...
	if err != nil || id <= 0 {
		response.Error(c, apperror.BadRequest("无效的AI模型ID"))
		return 0, false
	}
	return id, true
}
...
func createInput(req mutationRequest) CreateInput {
	return CreateInput{
		Name: req.Name, Driver: req.Driver, ModelCode: req.ModelCode, Endpoint: req.Endpoint, APIKey: req.APIKey, Status: req.Status,
	}
}
...
func (nilHTTPService) Init(ctx context.Context) (*InitResponse, *apperror.Error) {
	return nil, apperror.Internal("AI模型服务未配置")
}
