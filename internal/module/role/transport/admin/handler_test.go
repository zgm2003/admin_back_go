...
type fakeHTTPService struct {
	initResult *rolemodule.InitResponse
	listResult *rolemodule.ListResponse
	err        *apperror.Error

	listQuery   rolemodule.ListQuery
	createInput rolemodule.MutationInput
	updateID    int64
	updateInput rolemodule.MutationInput
	deleteIDs   []int64
	defaultID   int64
}
...
func (f *fakeHTTPService) List(ctx context.Context, query rolemodule.ListQuery) (*rolemodule.ListResponse, *apperror.Error) {
	f.listQuery = query
	return f.listResult, f.err
}

func (f *fakeHTTPService) Create(ctx context.Context, input rolemodule.MutationInput) (int64, *apperror.Error) {
	f.createInput = input
	return 12, f.err
}

func (f *fakeHTTPService) Update(ctx context.Context, id int64, input rolemodule.MutationInput) *apperror.Error {
	f.updateID = id
	f.updateInput = input
	return f.err
}
