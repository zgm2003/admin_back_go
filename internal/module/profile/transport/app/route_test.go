...
func TestAppProfileTransportPreservesProfileRoutes(t *testing.T) {
...
func TestAppProfileTransportDoesNotMountCurrentUserRoute(t *testing.T) {
	router := newAppProfileTestRouter(&fakeAppProfileService{}, &middleware.AuthIdentity{UserID: 7, Platform: enum.PlatformApp})

	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/api/app/v1/users/me", nil))
	if recorder.Code != http.StatusNotFound {
		t.Fatalf("expected profile transport not to mount app users/me, got %d body=%s", recorder.Code, recorder.Body.String())
	}
}
...
func TestAppProfileTransportRejectsWrongPlatformScope(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/api/app/v1/profile", nil))
	if recorder.Code != http.StatusUnauthorized {
		t.Fatalf("expected wrong platform status 401, got %d body=%s", recorder.Code, recorder.Body.String())
	}
}

func TestAppProfileTransportRejectsMissingProfileResult(t *testing.T) {
	router := newAppProfileTestRouter(&fakeAppProfileService{nilProfile: true}, &middleware.AuthIdentity{UserID: 7, Platform: enum.PlatformApp})

	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/api/app/v1/profile", nil))
	if recorder.Code != http.StatusInternalServerError {
		t.Fatalf("expected missing profile result to fail, got %d body=%s", recorder.Code, recorder.Body.String())
	}
}

func TestAppProfileTransportRejectsMissingProfileResultAfterUpdate(t *testing.T) {
	router := newAppProfileTestRouter(&fakeAppProfileService{nilProfile: true}, &middleware.AuthIdentity{UserID: 7, Platform: enum.PlatformApp})

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPut, "/api/app/v1/profile", strings.NewReader(`{"nickname":"App User 2","avatar":"avatar2.png","sex":2,"birthday":"2026-05-25","address_id":8,"detail_address":"湖北武汉光谷","bio":"new bio"}`))
	request.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(recorder, request)
	if recorder.Code != http.StatusInternalServerError {
		t.Fatalf("expected missing profile result after update to fail, got %d body=%s", recorder.Code, recorder.Body.String())
	}
}
