...
func TestOperationLogSkipsRoutesWithoutOperationRule(t *testing.T) {
	...
	router.POST("/api/v1/no-log", func(c *gin.Context) { c.String(http.StatusOK, "ok") })

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPost, "/api/v1/no-log", nil)
...
func TestOperationLogRecordsMatchedRouteAfterHandler(t *testing.T) {
	...
	router := newOperationLogTestRouter(OperationLogConfig{
		Rules: map[RouteKey]OperationRule{
			NewRouteKey(http.MethodPost, "/api/v1/permissions"): {Module: "permission", Action: "create", Title: "新增菜单"},
		},
		Recorder: func(ctx context.Context, input OperationInput) error {
			got = input
			return nil
	...
	router.POST("/api/v1/permissions", func(c *gin.Context) { c.String(http.StatusCreated, "created") })
	...
	if got.Method != http.MethodPost || got.Path != "/api/v1/permissions" || got.Status != http.StatusCreated || !got.Success {
...
func TestOperationLogMatchesGinFullPathForRouteParams(t *testing.T) {
	...
	router := newOperationLogTestRouter(OperationLogConfig{
		Rules: map[RouteKey]OperationRule{
			NewRouteKey(http.MethodDelete, "/api/v1/permissions/:id"): {Module: "permission", Action: "delete", Title: "删除菜单"},
		},
		Recorder: func(ctx context.Context, input OperationInput) error {
			got = input
			return nil
	...
	router.DELETE("/api/v1/permissions/:id", func(c *gin.Context) { c.String(http.StatusOK, "deleted") })

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodDelete, "/api/v1/permissions/9", nil)
	router.ServeHTTP(recorder, request)

	if recorder.Code != http.StatusOK {
		t.Fatalf("expected route to continue, got %d body=%s", recorder.Code, recorder.Body.String())
	}
	if got.Action != "delete" || got.Path != "/api/v1/permissions/:id" {
...
func TestOperationLogDoesNotBreakResponseWhenRecorderFails(t *testing.T) {
	router := newOperationLogTestRouter(OperationLogConfig{
		Rules: map[RouteKey]OperationRule{
			NewRouteKey(http.MethodDelete, "/api/v1/permissions/1"): {Module: "permission", Action: "delete", Title: "删除菜单"},
		},
		Recorder: func(ctx context.Context, input OperationInput) error {
