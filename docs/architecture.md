...
Selected direct Turns compile in ascending User Message ID order as their original user and assistant roles; each native historical attachment remains in its owning user message, and the current user message is always the final user-role message.
Recent native historical attachments are emitted once as attachment parts. Their duplicate textual fact envelope is omitted; older ready private attachments that are not replayed natively retain the bounded fact envelope instead of disappearing.
New Plans persist only role-boundary byte offsets into the existing bounded Turn content snapshot. They do not duplicate conversation text in metadata. Plans created before this projection have no conversation_turn metadata and retain their byte-stable legacy compilation path.
