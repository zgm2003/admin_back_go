...
支付域当前收敛为 `internal/module/payment` bounded context，但 V1 active scope 只有支付宝配置：
...
```text
config-only slice 不关闭订单、不同步订单、不处理回调。
如果后续做支付订单 slice，必须重新写 spec/plan 再恢复订单补偿任务。
