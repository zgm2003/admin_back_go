...
支付域当前收敛为 `internal/module/payment` bounded context，active scope 是支付宝配置 + 第一版支付宝订单：

```text
internal/module/payment                 # 支付宝配置 CRUD、证书上传、本地配置测试、订单 create/pay/sync/close
internal/platform/payment               # 证书私有存储、路径解析、支付网关接口
internal/platform/payment/alipay        # 唯一允许接入支付宝 SDK 的平台层
payment_configs                         # 支付配置 active runtime 表
payment_orders                          # 支付订单 active runtime 表
```
...
```text
payment V1 范围是 Alipay config 和 Alipay order pay；wallet、notify callback、event、refund、withdraw、reconcile、WeChat 或业务履约都不在本 slice。
配置事实源是 payment_configs；订单事实源是 payment_orders；旧 channel/event 表已经从 live DB 清掉，不再作为 Go/Vue active runtime 或冷启动参考。
return_url 不属于 payment_configs，只属于创建具体 payment_orders 时的单次入参。
...
菜单路径只保留 /payment/config 和 /payment/orders；旧 channel/event/pay/wallet 菜单必须从 users/init router 消失。
provider 是当前字段合同的一部分，但只允许 alipay；merchant_id、sign_type、extra_config 不属于当前字段合同。
```

支付 cron 在当前 payment order Alipay pay v1 slice 中仍不注册：
...
```text
当前订单 slice 支持手动关闭订单、手动同步订单，不处理 notify callback。
自动关闭、自动同步、退款、对账或履约重试必须另写 spec/plan，再决定 cron、索引和事件表。
