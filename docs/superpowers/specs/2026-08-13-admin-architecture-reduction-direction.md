...
角色权限编辑采用“页面可选、动作从属页面”的权限矩阵，不恢复难以横向扫描的级联树，也不把页面访问伪装成一个普通按钮。
...
- 页面名称前的 checkbox 直接表示页面访问权限；
- 动作列只展示真实按钮权限，不再额外显示“页面访问”；
- 勾选任一按钮时自动勾选所属页面；
- 取消页面时同时清除该页面下的按钮权限；
- 取消最后一个按钮时保留页面权限，除非用户主动取消页面；
- 没有按钮的页面仍可直接勾选页面；
- 目录只负责分组，不保存到 `role_permissions`；
- 全选、清空、半选统计继续使用页面和按钮的真实权限 ID；
- 不增加新的页面访问权限码，不修改现有接口和数据库结构。

该 UI 只是更准确地表达现有数据结构。后端继续在保存时保证：选择按钮必然包含其有效页面父权限。
...
旧基线曾验证过的初始化事实（仅作历史记录，不属于当前执行输入）：
...
- 本地 MySQL 是当前唯一数据库事实，仓库不承诺从空库初始化；
