...
> **2026-07-25 最终归档结论：** 本文只保留首次细审的历史问题与整改依据，不是当前产品契约。`PD-05`、`PD-06` 所述跨实例 billing block 已从精简后的阶段 A 移除；`PD-07` 已确定为失败 attempt 即使返回完整 usage 也不向用户收费；`PD-08` 不在阶段 A 改写对象保留策略，用户删除和平台损坏均不产生 AI 退款。当前主 Spec 已获技术架构复核通过，可以据此编写分阶段 implementation plan；下文任何旧的截止时间、`uncertain`、自动对账或退款建议均不得用于实现。
...
细审时发现，用户主动删除与平台永久损坏必须在状态机中分离；当前产品政策已固定为两者都不产生 AI 退款，平台永久损坏只转为 `unavailable`：

`internal/module/ai/image/repository.go:155`

如果不区分删除原因，会产生两类错误：

1. 用户下载结果后主动删除，再被完整性检查误判为平台损坏并错误改变原始财务事实；
...
- 平台完整性损坏才允许 `finalized -> unavailable`；不退款、不重新生成、不修改旧账单或钱包流水；
...
## 4. 产品决策状态（2026-07-25 更新）

以下首次细审时提出的四项已由用户确认，并已写入主 Spec：

1. 成功文本和媒体结果不设 TTL。
2. 平台永久损坏任何已扣款结果不退款，不重新生成。
3. 图片部分成功后，部分输出永久损坏也不退款。
4. 绝对恢复截止固定为：对话、文本、工具、音频 15 分钟；图片 1 小时；视频 6 小时。

原 `PD-05` 至 `PD-08` 已全部退出待决状态：

1. 阶段 A 不实现跨实例自动 billing block，也不新增解除权限。
2. 供应商明确失败的 attempt 不向用户收费；完整 usage 只留作审计。
3. 阶段 A 不改变现有对象保留/清理策略；用户删除只改变可见性，不退款、不重新生成。
...
- 不可变计费明细只在最终金额确定后形成；
- 用户删除与平台损坏不会混淆，且二者都不产生 AI 退款或财务反转；
- 媒体存储身份在配置切换后仍能定位旧结果；
