...
19. 已经形成终态的生成请求必须可由首次请求身份重放其终态；重放不能再次调用供应商或再次扣款。成功结果不设 TTL；平台结果永久损坏只转为不可用，不产生 AI 退款或再次生成。
20. 供应商结果不确定、平台 finalizer 暂时失败或媒体对象处于候选状态时，都必须使用首次派发时固化的持久化截止时间和有界恢复，不能因重启续期或无限冻结用户余额；对话、文本、工具和音频为 15 分钟，图片为 1 小时，视频为 6 小时。
...
| ID | 已确认决策 | 固定规则 |
| --- | --- | --- |
| `PD-01` | 成功文本和媒体结果不设 TTL | 仅用户主动删除可改变可见性；同一 `request_id` 始终重放原终态 |
| `PD-02` | 平台确认永久损坏的已扣款结果不退款 | 只转为 `unavailable`，不重新生成、不二次扣款、不创建 AI 入账流水 |
...
- `SourceAIGenerate` 与未接业务的 `SourceAIRefund` 当前都存在；本期必须删除 `SourceAIRefund` 及其 AI 退款字典、服务、测试和文档残留，AI 运行时只允许生成扣款与未扣款冻结释放。
...
- `wallet`：拥有余额、冻结、补充冻结、扣款、释放和钱包流水规则；本期不提供 AI 退款能力。
...
供应商 API Key 账号是正式的一等上游接入：驱动使用账号已有的 `base_url` 与 `api_key` 调用 OpenAI-compatible、官方 OpenAI 或聚合平台，不为对账另建 API Key、环境变量或“对账密钥”。供应商凭据继续使用现有账号凭据保护机制；平台内部派生/加密继续复用既有 `APP_SECRET` 根密钥体系，Run、账单、attempt、日志和对象引用均不得保存 API Key 明文。

`base_url + api_key` 只解决认证和请求转发，不能被误认为天然具备逐请求对账能力。每个驱动在代码中声明不可由管理员伪造的对账能力快照，并在首次派发时写入 provider-attempt：

| 能力 | 可作为结算证据 | 规则 |
| --- | --- | --- |
| `response_usage` | 是 | 同步响应或 SSE/WebSocket 终止事件返回完整、可校验 usage；这是默认首选证据 |
| `task_status_usage` | 是 | 异步媒体的官方任务状态返回该任务完整 usage，且能与保存的 provider task ID 精确对应 |
| `request_lookup_usage` | 是 | 驱动已实现且文档化的逐请求查询，使用持久化的上游请求/响应 ID 返回精确 usage 或明确未执行 |
| `idempotent_replay` | 仅用于安全恢复调用边界 | 供应商明确保证稳定 idempotency key 的重放语义时，才可用原 key 恢复同一次调用；重放本身不等同于 usage 证据，只有重放响应返回的完整 usage 才按 `response_usage` 取证 |
| `aggregate_usage` / `balance_only` / `none` | 否 | 仅可作运营诊断、账号健康或预警，不能据此结算、释放或证明未执行 |

attempt 至少持久化 stable provider idempotency key、`provider_request_id`、`provider_response_id`、异步 `provider_task_id`、响应哈希、原始响应头中可用的上游关联 ID、归一化原始 usage、`usage_evidence_kind`、`usage_evidence_ref`、`usage_observed_at`、完整性和 usage fingerprint。上游请求 ID 只用于关联和受支持的精确查询，不能假定任意 OpenAI-compatible 上游都能通过它查询账单。

对 `sub2api` 这类聚合上游，`GET /v1/sub2api/billing` 只表示该 API Key 当前倍率，`GET /v1/usage` 只表示余额及聚合统计；两者都属于 `aggregate_usage` / `balance_only`，绝不能用作单个 Run 的结算证据。上游余额也不能替代我方 Hold，因为并发请求会改变余额且上游没有代表我方钱包的原子预占。若未来某个已审核驱动提供真正的逐请求状态/usage 接口，才以该驱动的 `request_lookup_usage` 合约接入，不允许根据任意 `base_url` 猜路径或探测私有接口。

Runner 在流式客户端断开后继续读取已派发上游的终止事件，以争取 `response_usage`；客户端断开只停止本次等待和输出，不能自动撤销任务 owner 或启动无人接收的 failover。发生进程崩溃且没有持久化完整 usage 时，只能按上述受支持证据恢复；仅当驱动已声明并实现 `idempotent_replay` 时，才可用原 key 重放并从返回结果取得新的 `response_usage`，否则不得以新调用试探。聚合用量、余额变化、估算金额和“通常不会扣费”的经验都不是证据。截止前保持 `outcome_unknown/uncertain`，截止后 `unbilled` 并释放 Hold，绝不捕获估算金额。这是未收费释放，不是退款。
...
不完整或不合法用量不能全部按普通输入高价扣款。若供应商已经返回有效内容，账单进入 `unbilled`、释放冻结，用户仍获得完整回复，平台承担异常成本。若没有形成有效结果，则按失败流程结束。两种情况在已取得有效上游结果时都记录第 5.6 节定义的计费完整性异常，并立即暂停该 `billing_safety_key`；单纯进程/网络导致、尚未取得结果的 `outcome_unknown` 不适用这条暂停规则。
...
`total_consume_units` 只累计实际 AI 扣款等出账。用户删除、结果永久损坏、`unbilled` 和释放冻结都不得回写、减少或改写历史毛消费；本期不存在 AI 退款入账流水。
...
每个冻结使用唯一 `(source_type, source_id)`，AI 的 `source_id` 是用量账单 ID。最终扣款只使用 `SourceAIGenerate`；释放未扣款冻结不是退款，不产生钱包收入流水。用户删除、结果永久损坏和部分输出损坏均不改写已结算账单、原扣款流水或钱包余额。

冻结、补充、捕获和释放都锁定钱包行并保持幂等。钱包规则只存在于 `wallet` 模块，`usagecharge` 和 AI 服务不得复制余额算法。

Token 请求临时冻结按普通输入、缓存写入、缓存读取中的最高适用价格估算；最终只按真实互斥分类扣款并释放差额。每个请求必须有最大输出 Token，未提供时使用产品上限与目录模型上限中的较小值；工具调用轮次、工具输出和每轮模型输出都必须有产品硬上限。

每次新的真实供应商调用都必须先满足同一个冻结充足性不变量：
...
普通重试不在首次执行时冻结全部 `max_attempts` 的最坏总额；Runner 在每个新 `attempt_no` 派发前，把同一个 Charge 对应的 Hold 补充到上述累计目标。`PD-07` 未确认或某个失败 attempt 尚未完成最终收费分类时，其完整实际用量按“仍可能收费”计入冻结目标，这只是防止透支的预占，不代表最终一定扣款。先前 attempt 用量不完整时禁止进入普通重试，除非供应商权威证据证明该 attempt 未执行或没有可计费用量，此时它对累计目标贡献为零。

能够对一次业务执行波次及其全部允许工具轮次形成安全上界时，首次冻结可覆盖该波次，后续每轮只校验现有 Hold 是否充足；无法形成整次上界时，必须在每个工具轮次的真实调用前按同一公式补充。当前 Hold 已达到目标时不重复增加；不足时只 top-up 差额。任何 reserve/top-up 失败都不得持久化新的 `dispatched` 事实或发送网络字节。

目录为模型绑定受控冻结估算 profile。估算器接收最终序列化供应商请求并返回保守输入上界，禁止使用通用字符数随意猜测。缺少经过测试的估算 profile 或输出上限时，模型不能进入 billing-ready。

按轮次或普通重试补充冻结时，必须在每次追加模型调用前完成。余额不足则停止下一次供应商调用，并由统一 finalizer 按此前已经发生且能够准确取得的实际用量和 `PD-07` 结算政策终结；Run/任务失败且不交付半成品，不能释放全部冻结后把此前可能应收费的供应商成本变成免费调用，也绝不先调用再透支。
...
两者必须在首次派发前由目录/运行策略解析并写入账单快照；重新 claim、进程重启、Worker 迁移和供应商轮询都不能重算或延长 `resolution_deadline_at`。`absolute_resolution_ttl` 固定为对话、文本、工具和音频 15 分钟，图片 1 小时，视频 6 小时；这是 billing-ready 的发布约束，不是示例值。
...
普通可重试失败不终结 Run：命令回到 `pending`，Run 保持 `running`，并追加一次幂等的 `retry_scheduled` Run Event。下一次真实供应商调用使用新的 `attempt_no`；只有供应商明确保证幂等重放时才复用原 provider idempotency key。重试等待期间 Charge 与非零 Hold 保持非终态，不能先 `released` 后复活账单。每个新 `attempt_no` 都必须在 `dispatched` 前按第 9.3 节累计公式重新报价并 reserve/top-up 同一个 Hold；补充失败时不得派发。首次执行时快照 `max_attempts`，重新 claim 和重启不得重置已消耗次数。达到 `max_attempts`、不可重试错误或恢复硬截止后，统一 finalizer 才终结 Command、Run、Charge、Hold 和 durable failed/canceled event；`resolution_deadline_at` 不因重试或重启延长。各 attempt 的原始用量全部保留，最终是否把失败 attempt 纳入用户扣款由 `PD-07` 决定。
...
`settled`、`released`、`rejected`、`unbilled` 是终态。`unbilled` 只表达“本次没有捕获用户资金”，不单独表达业务成功或失败：有完整有效结果时，业务任务/命令和 Run 可以成功或取消，结果按对应规则发布或丢弃；没有可靠结果时，业务任务/命令和 Run 失败或取消且不交付结果。查询与审计必须联合业务状态、结果状态和账单状态判断，不能把所有 `unbilled` 都显示成成功或失败。任何已结算 AI 账单均不通过后续钱包入账反转。
...
- `ai_image_tasks` 增加幂等字段、显式 Run 绑定、结果状态、期望/实际输出数量和 manifest 摘要；现有 `ai_image_files` 继续作为输入、遮罩和输出结果事实。输出行增加 `publish_state`、内容 SHA-256 和验证元数据，多张输出全部进入候选 manifest 后才允许 finalizer；官方允许部分成功时，manifest 明确实际成功集合并只按该集合结算。用户删除与平台损坏分别记录，不能复用同一个 `is_del` 推导结果可用性或财务动作。
...
任务显示 `success` 但正文、manifest 或对象引用经确认永久缺失/摘要不符时，先幂等转为 `unavailable` 并返回 `error.code=ai.result_unavailable`，绝不能借同一 ID 重新生成、退款或改写原扣款。对象存储暂时不可用时只返回可重试 dependency 错误，不提前判定永久损坏。

成功任务、结果正文和 `finalized` 媒体对象不设 TTL，也不得配置后台清理或对象存储生命周期规则。用户主动删除只改变可见性；其物理对象处理仍等待 `PD-08` 决定。输入附件继续遵守既有存储生命周期，但不能误删仍被结果重放引用的对象。
...
- 每次响应都保存供应商请求 ID、响应 ID、异步 task ID、响应哈希、经过校验的归一化原始用量、`usage_evidence_kind/ref/observed_at`、完整性和用量指纹；失败、取消与 `outcome_unknown` 也保存明确状态，不能因为业务没有结果就丢弃供应商返回的 usage；
- `prepared` 且没有 `dispatched_at` 可以由当前合法执行 owner 使用原 key 派发；任何已经带 `dispatched_at` 的尝试都视为“可能已到达供应商”；
- 恢复只能复用同一调用身份。只有供应商文档明确保证该 key 的幂等重放，或本驱动的逐请求权威查询证明上次没有执行时才可再次发送；余额、聚合 usage 或倍率接口不能证明未执行。否则保持不确定并最终按本设计终结，禁止换新 key 盲目重发；
- 已证明未形成成功结果的普通可重试错误把业务任务重新排队，但原 Run、Charge、价格/倍率快照、Hold 和首次 `resolution_deadline_at` 保持不变；下一次真实供应商调用分配新的 `attempt_no`，Run 原子追加 `retry_scheduled` 事件，并在 `dispatched` 前按“此前仍可能收费的完整实际用量金额 + 下一次调用保守上界”补充同一个 Hold。补充失败不得派发。
...
- 工具调用后准备下一次模型请求前，先持久化并归一化已完成调用的原始用量；首次冻结未覆盖整次上界时，按第 9.3 节累计目标补充同一冻结，并在补充成功后才写下一次 `dispatched`；
...
3. 聊天 finalizer 锁定命令、候选、账单和钱包，先在锁内判定取消/完成竞态赢家，再按以下优先级提交唯一组合：
   - 取消未胜出且用量完整：在 finalizer 内一次性插入不可变 `ai_usage_charge_items`；扣款大于零时 capture 精确金额、释放差额并写一条钱包流水，扣款为零时释放全部冻结且不写零金额流水；账单均设为 `settled`；
   - 取消未胜出且内容有效但用量不完整：只为已经验证的已知用量项插入实际扣款为零的明细，缺失分类不造行、不猜数量或金额；释放全部冻结、账单设为 `unbilled`，并写入持久化计费完整性异常与该 `billing_safety_key` 的首次 active block；
   - 取消请求胜出且用量完整：按完整实际用量将账单设为 `settled` 并释放差额，已有候选设为 `discarded`，命令和 Run 设为 `canceled`，原子追加 Run canceled event；扣款为零仍使用零金额 `settled`，不是 `released`；
   - 取消请求胜出且供应商权威证明未执行或没有可计费用量：释放冻结、账单设为 `released`，已有候选设为 `discarded`，命令和 Run 设为 `canceled`，原子追加 Run canceled event；
   - 取消请求胜出、用量不完整且尚未到 `resolution_deadline_at`：不写最终 `ai_usage_charge_items`，账单设为或保持 `uncertain`，继续保留 Hold；已有候选设为 `discarded` 并清空正文，命令和 Run 必须设为 `outcome_unknown`，保留 `cancel_requested_at`，只追加 outcome-unknown Run Event，不得提前追加取消终态事件；
   - 取消请求胜出、到达 `resolution_deadline_at` 仍无完整可靠用量：只为已验证的已知用量项写零实际扣款明细，释放冻结、账单设为 `unbilled`，命令和 Run 设为 `canceled`，追加 Run canceled event；
4. 取消未胜出的两个分支才允许发布内容：插入可见 assistant message，绑定 reply command 与 `ai_run.assistant_message_id`，将回复命令设为 `succeeded`、Run 设为成功，原子追加 Run success event 和 durable `ai.response.completed.v1`；取消胜出的所有分支都不得插入 assistant message 或增加未读。
5. 成功发布或确认丢弃时，将完成候选设为 `finalized`/`discarded` 并清空候选正文；取消待对账分支虽然资金尚未终结，但内容已经确定永不发布，因此也立即 `discarded` 并清空正文，后续只恢复 usage/账单事实。
6. 事务提交后才 best-effort 实时推送对应的权威状态事件。
...
finalizer 暂时失败时保留冻结，并从持久化候选重试相同完成事实，不重复调用供应商。数据库死锁、超时或短暂不可用属于可重试 finalizer 故障，必须回滚整个事务，不能被解释成“补充冻结失败”而提前改变资金事实。取消已胜出且用量不完整时，在持久化的 `resolution_deadline_at` 之前只能保持 `outcome_unknown/uncertain`，不得提前 `unbilled/released`。到达截止后：完整可靠用量仍按实际金额 `settled`；有效内容但用量不完整时进入 `unbilled`，未取消则免费发布、取消已胜出则保持丢弃并进入取消终态；没有可靠候选时把业务置为对应取消/失败终态、账单置为 `unbilled` 并释放冻结。若供应商已经响应、但完成候选尚未成功落库时进程崩溃，则进入不确定恢复；最终只能根据驱动支持的逐请求查询、持久化终止事件或其他精确证据取得真实用量和结果。余额、聚合 usage、倍率和估算都不得替代该证据；缺失证据时在截止后释放且不扣估算金额，不能盲目重发。

不可重试失败、耗尽重试、取消和超时也使用同一个 finalizer：在同一事务内根据已持久化的真实用量决定 `settled`/`released`/`unbilled` 终态，或进入 `outcome_unknown/uncertain` 中间态。只有金额终态才插入最终计费明细并追加对应 durable 失败/取消/超时终态事件；`uncertain` 不得伪装成终态或提前写取消终态事件。普通可重试错误在达到终止条件前不得走终态分支。禁止由 chat service、Run recorder 和 wallet 各自 best-effort 写终态。
...
停止与完成并发时由命令租约、fencing token、行锁和一次终态转换决定唯一赢家：停止请求先写入 `cancel_requested_at` 时，Worker 必须尝试供应商取消；对于已派发流式调用仍继续 drain 终止事件，并在支持时查询该任务/请求的权威 usage，不能立即把已派发命令改成免费取消终态。取消 finalizer 胜出后拒绝发布内容，但对完整合法用量照常结算；用量不完整则保持 `outcome_unknown/uncertain` 到对账截止。完成事务先提交时返回既有成功结果，迟到停止不能撤销已结算事实。
...
如果实际费用意外超过保守冻结，finalizer 在同一钱包锁内计算差额：可用余额足够时补充同一 hold 后正常捕获；锁内确定余额不足时，不能部分捕获或透支，结果按不捕获估算金额的 `unbilled` 规则处理并释放已有冻结。无论差额是否补充成功，超额都必须持久化为计费完整性异常并立即形成 active block；解除按 `PD-06`。数据库异常不能被当作余额不足，也不能把“调用后再负数扣款”当作正常补差路径。
...
- 单价、分项金额、冻结和异常恢复状态；
- 结果可用性、用户删除/平台损坏原因、上游请求/响应关联 ID、usage 证据来源和 billing safety block 诊断；不展示不存在的 AI 退款流水。
...
- 部分成功按实际成功数量和真实用量结算；部分输出的用户删除与平台永久损坏分开记录，损坏不退款也不改写原账单；
...
- 缺少官方计费所需数据时 `unbilled`，不能用响应字节数猜时长或 Token；已取得有效上游结果时必须保存计费完整性异常并立即形成该 `billing_safety_key` 的 active block，进程/网络 `outcome_unknown` 仍只走请求级对账。
...
Runner 执行
  ...
  -> 构造本次最终供应商请求并计算保守费用上界
  -> 按累计冻结目标 reserve 或 top-up 同一个 Hold
  -> 创建或恢复本次 provider-attempt prepared
...
普通重试、用户取消、`outcome_unknown`、finalizer 重试和绝对截止恢复都回到这条路径。任何新 `attempt_no` 或工具轮次只有在 pricing/block/storage 检查、报价和同一 Hold 的 reserve/top-up 全部成功后才能进入 `dispatched`；恢复本地 candidate/finalizer 不重新报价或调用供应商。HTTP 请求 context 只控制等待和响应写入，不拥有供应商执行；同一任务在任意时刻只能有一个有效 lease/fencing owner。
...
恢复器先扫描聊天 `staged` 完成候选和各模态已经持久化的结果候选，只重试本地 finalizer；随后扫描超过恢复期限的非终态任务、provider-attempt、账单和冻结。所有扫描使用租约、fencing token 及有界批次幂等恢复。恢复只能依据持久化真实内容/用量结算，依据已证明未执行/无用量释放，或在截止后以 `unbilled` 释放；禁止用新 key 或未声明幂等语义的调用盲目重调已派发且结果不确定的供应商请求。只有第 5.7 节已声明的 `idempotent_replay` 驱动才可用原 key 恢复同一次调用，并且必须从返回结果取得完整 `response_usage` 后才能结算。
...
- 各模态固定恢复期限（对话、文本、工具、音频 15 分钟；图片 1 小时；视频 6 小时）、绝对截止时间不可被重启延长、到期资金释放以及迟到结果丢弃；
- 成功聊天原子提交消息、Run、Run Event、账单、钱包和 durable completed 事件；
- 取消未胜出的完整有效结果在用量不完整时按 `unbilled` 发布，且不重复扣缓存 Token；取消已胜出时仍丢弃内容；
...
- `response_usage`、异步 `task_status_usage`、受支持的 `request_lookup_usage` 分别只在返回完整可校验 usage 时结算；`sub2api` 的倍率、余额和聚合 `/v1/usage` 响应不能作为单请求结算或未执行证明；
- 客户端断开后已派发流式请求继续 drain 终止事件；进程崩溃且缺少完整 usage 时保持不确定到截止，不按估算、余额差或聚合统计扣款；
...
- 已确认永久缺失或摘要不符时只转一次 `unavailable` 并稳定返回 `ai.result_unavailable`；临时对象存储故障不误判，且不退款、不重新生成或改写原账单；
- 用户删除只转为 `user_deleted`，不退款且同一请求不重新生成；`PD-08` 确认后再测试物理对象清理时机；
- `success/finalized` 和仍待 finalizer 的 `staged` 候选不被后台 TTL 清理，`discarded`/孤儿对象清理不误删活动或正式结果；
...
10. 发送、编辑、重新生成、普通重试和工具轮次都在每次 Worker 真实调用前完成价格检查、报价及充足冻结；成功结果统一原子完成。
...
21. 文本、工具草稿、图片、视频和音频的成功结果不设 TTL，且都可由首次请求身份重放；HTTP 断开或响应丢失不终止 durable owner，也不重复调用供应商或扣款。
